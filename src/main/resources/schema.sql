...
CREATE TABLE UNI_JOB (

    JOB_TYP VARCHAR(40),
    DAT     CHAR(8),

    CONSTRAINT PK_UNIJOB PRIMARY KEY (JOB_TYP)
);
...
CREATE TABLE RESP_REC_SEND_LST (

    OPEN_ID         VARCHAR(200),
    DPRT_NAM        VARCHAR(200),
    MAIL_ADDR       VARCHAR(2000),
    MAIL_SUBJ       VARCHAR(2000),
    LAST_SEND_TMSTP DATETIME,
    ERR_MSG         VARCHAR(2000),

    CONSTRAINT PK_RESPRECSENDLST PRIMARY KEY (OPEN_ID, DPRT_NAM)
);
