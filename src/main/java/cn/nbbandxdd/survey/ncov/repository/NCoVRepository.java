    ...
    @Query(
        "(" +
            "SELECT '天津研发部' AS DPRT_NAM, " +
                ...
                "IFNULL(SUM(CASE WHEN Q01 LIKE '到岗%' AND HAM_CD NOT LIKE 'TY%' THEN 1 ELSE 0 END), 0) AS ON_CNT_E, " +
                "IFNULL(SUM(CASE WHEN Q01 NOT LIKE '到岗%' AND HAM_CD NOT LIKE 'TY%' THEN 1 ELSE 0 END), 0) AS OFF_CNT_E, " +
                "IFNULL(SUM(CASE WHEN Q01 LIKE '请假%' AND HAM_CD NOT LIKE 'TY%' THEN 1 ELSE 0 END), 0) AS LEAVE_CNT_E, " +
                "IFNULL(SUM(CASE WHEN Q01 LIKE '离开驻地%' AND HAM_CD NOT LIKE 'TY%' THEN 1 ELSE 0 END), 0) AS AWAY_CNT_E, " +
                "IFNULL(SUM(CASE WHEN Q01 LIKE '因居住地临时要求%' AND HAM_CD NOT LIKE 'TY%' THEN 1 ELSE 0 END), 0) AS TEMP_CNT_E, " +
                "IFNULL(SUM(CASE WHEN Q01 LIKE '%（涉及灰名单）%' AND HAM_CD NOT LIKE 'TY%' THEN 1 ELSE 0 END), 0) AS GRAY_CNT_E " +
