...
public interface NCoVRepository extends ReactiveCrudRepository<NCoVEntity, String> {
    ...
    @Query("""
        ( \
            SELECT '天津研发部' AS DPRT_NAM, \
                ...
                IFNULL(SUM(CASE WHEN Q12 = '阳性' AND FLAG = 'I' THEN 1 ELSE 0 END), 0) AS APOS_CNT_I, \
                IFNULL(SUM(CASE WHEN Q11 = '阳性' AND FLAG = 'I' THEN 1 ELSE 0 END), 0) AS NPOS_CNT_I, \
                ...
                IFNULL(SUM(CASE WHEN Q12 = '阳性' AND FLAG = 'E' THEN 1 ELSE 0 END), 0) AS APOS_CNT_E, \
                IFNULL(SUM(CASE WHEN Q11 = '阳性' AND FLAG = 'E' THEN 1 ELSE 0 END), 0) AS NPOS_CNT_E, \
                ...
                IFNULL(SUM(CASE WHEN Q12 = '阳性' AND FLAG = 'V' THEN 1 ELSE 0 END), 0) AS APOS_CNT_V, \
                IFNULL(SUM(CASE WHEN Q11 = '阳性' AND FLAG = 'V' THEN 1 ELSE 0 END), 0) AS NPOS_CNT_V \
            FROM NCoV WHERE DPRT_NAM IN ('天研开发一部', '天研开发二部', '天研应用平台研发部', '天研测试部', '天研综合管理部') \
        ) \
            UNION ALL \
        ( \
            SELECT T1.DPRT_NAM, \
                IFNULL(T2.ON_CNT_I, 0) AS ON_CNT_I, IFNULL(T2.OFF_CNT_I, 0) AS OFF_CNT_I, IFNULL(T2.LEAVE_CNT_I, 0) AS LEAVE_CNT_I, IFNULL(T2.AWAY_CNT_I, 0) AS AWAY_CNT_I, IFNULL(T2.TEMP_CNT_I, 0) AS TEMP_CNT_I, IFNULL(T2.GRAY_CNT_I, 0) AS GRAY_CNT_I, IFNULL(T2.APOS_CNT_I, 0) AS APOS_CNT_I, IFNULL(T2.NPOS_CNT_I, 0) AS NPOS_CNT_I, \
                IFNULL(T2.ON_CNT_E, 0) AS ON_CNT_E, IFNULL(T2.OFF_CNT_E, 0) AS OFF_CNT_E, IFNULL(T2.LEAVE_CNT_E, 0) AS LEAVE_CNT_E, IFNULL(T2.AWAY_CNT_E, 0) AS AWAY_CNT_E, IFNULL(T2.TEMP_CNT_E, 0) AS TEMP_CNT_E, IFNULL(T2.GRAY_CNT_E, 0) AS GRAY_CNT_E, IFNULL(T2.APOS_CNT_E, 0) AS APOS_CNT_E, IFNULL(T2.NPOS_CNT_E, 0) AS NPOS_CNT_E, \
                IFNULL(T2.ON_CNT_V, 0) AS ON_CNT_V, IFNULL(T2.OFF_CNT_V, 0) AS OFF_CNT_V, IFNULL(T2.LEAVE_CNT_V, 0) AS LEAVE_CNT_V, IFNULL(T2.AWAY_CNT_V, 0) AS AWAY_CNT_V, IFNULL(T2.TEMP_CNT_V, 0) AS TEMP_CNT_V, IFNULL(T2.GRAY_CNT_V, 0) AS GRAY_CNT_V, IFNULL(T2.APOS_CNT_V, 0) AS APOS_CNT_V, IFNULL(T2.NPOS_CNT_V, 0) AS NPOS_CNT_V \
    ...
    @Query("SELECT DPRT_NAM, GRP_NAM, USR_NAM, HAM_CD, (CASE WHEN Q01 LIKE '涉疫%' THEN CONCAT('涉疫（抗原：', IFNULL(Q12, '暂无'), '，核酸：', IFNULL(Q11, '暂无'), '）') ELSE Q01 END) AS Q01 FROM NCoV WHERE DPRT_NAM = :dprtNam AND GRP_NAM = :grpNam ORDER BY HAM_CD")
